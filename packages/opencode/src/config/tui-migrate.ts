...
const TUI_SCHEMA_URL = "https://supercode.ai/tui.json"
...
export async function migrateTuiConfig(input: MigrateInput) {
  const files = await supercodeFiles(input)
...
async function supercodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "supercode"),
    ...(await Filesystem.findUp(["supercode.json", "supercode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "supercode"))
  }
  if (Flag.SUPERCODE_CONFIG) files.push(Flag.SUPERCODE_CONFIG)
