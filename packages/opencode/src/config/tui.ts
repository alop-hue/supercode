...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // Every config dir we may read from: global config dir, any `.supercode`
  // folders between cwd and home, and OPENCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.SUPERCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  if (Flag.SUPERCODE_TUI_CONFIG) {
    const configFile = Flag.SUPERCODE_TUI_CONFIG
  ...
  // 4. `.supercode` directories (and OPENCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".supercode") || dir === Flag.SUPERCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".supercode") && dir !== Flag.SUPERCODE_CONFIG_DIR) continue
