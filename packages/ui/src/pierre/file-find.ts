...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("supercode-find")
  api.delete("supercode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("supercode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("supercode-find", new Highlight(...rest))
    return true
  }
